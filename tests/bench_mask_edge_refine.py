...
from core.mask_edge_refine import edge_aware_refine  # noqa: E402

SEED = 20260421
WARMUP_ITERS = 3
MEASURE_ITERS = 10
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_mask_edge_refine.json"
...
class BenchCase:
    ...
    @property
    def name(self) -> str:
        return f"cpu-{self.resolution}-b{self.batch}-guided_filter"


CASES = (BenchCase(512, 1), BenchCase(1024, 1))
...
def _run_case(case: BenchCase) -> dict[str, float | int | str]:
    mask = _rand((case.batch, case.resolution, case.resolution), 11)
    guide = _rand((case.batch, case.resolution, case.resolution, 3), 23)

    def run_once() -> torch.Tensor:
        return edge_aware_refine(mask, guide, radius=4, eps=1e-3, feather_sigma=0.0)
    ...
    return {
        "case": case.name,
        "module": "mask_edge_refine",
        "scenario": "guided_filter",
        "device": "cpu",
...
def test_bench_mask_edge_refine(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
