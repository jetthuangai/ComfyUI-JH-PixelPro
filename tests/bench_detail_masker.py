...
from core import high_freq_detail_mask  # noqa: E402
...
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_detail_masker.json"
...
CASES = [
    BenchCase("cpu-512-laplacian-rel", "cpu", 512, "laplacian", "relative_percentile", 2, 10),
    BenchCase("cpu-1024-laplacian-rel", "cpu", 1024, "laplacian", "relative_percentile", 1, 10),
    BenchCase("cpu-2048-laplacian-rel", "cpu", 2048, "laplacian", "relative_percentile", 0, 10),
    BenchCase("cpu-1024-sobel-rel", "cpu", 1024, "sobel", "relative_percentile", 1, 10),
    BenchCase("cpu-1024-fsgauss-rel", "cpu", 1024, "fs_gaussian", "relative_percentile", 1, 10),
    BenchCase("cpu-1024-laplacian-abs", "cpu", 1024, "laplacian", "absolute", 1, 10),
    BenchCase("gpu-512-laplacian-rel", "cuda", 512, "laplacian", "relative_percentile", 2, 10),
    BenchCase("gpu-1024-laplacian-rel", "cuda", 1024, "laplacian", "relative_percentile", 1, 10),
    BenchCase("gpu-2048-laplacian-rel", "cuda", 2048, "laplacian", "relative_percentile", 0, 10),
]
...
def render_markdown(hardware: dict[str, str], results: list[BenchResult]) -> str:
    lines = [
        "# High-Frequency Detail Masker Benchmark",
        "",
        "## Hardware",
        "",
        ...
        "",
        (
            "| case | device | resolution | kernel_type | threshold_mode | status | "
            "reason | min_ms | median_ms | mean_ms |"
        ),
        "|---|---|---:|---|---|---|---|---:|---:|---:|",
    ]
    ...
    for row in results:
        lines.append(
            f"| {row.name} | {row.device} | {row.resolution} | {row.kernel_type} | "
            f"{row.threshold_mode} | {row.status} | {row.reason or '—'} | "
...
def test_bench_detail_masker(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
...
def main() -> int:
    ...
    logging.getLogger("core.detail_masker").setLevel(logging.ERROR)
