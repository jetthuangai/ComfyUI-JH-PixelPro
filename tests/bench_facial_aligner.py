...
from core import facial_align  # noqa: E402
from core.facial_aligner import canonical_landmarks  # noqa: E402
...
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_facial_aligner.json"
...
CASES = [
    BenchCase("cpu-512", "cpu", 512, 2, 10),
    BenchCase("cpu-768", "cpu", 768, 1, 10),
    BenchCase("cpu-1024", "cpu", 1024, 1, 10),
    BenchCase("gpu-512", "cuda", 512, 2, 10),
    BenchCase("gpu-768", "cuda", 768, 1, 10),
    BenchCase("gpu-1024", "cuda", 1024, 1, 10),
]
...
def _make_image(device: torch.device) -> torch.Tensor:
    generator = torch.Generator(device="cpu")
    generator.manual_seed(SEED)
    image = torch.rand((1, 3, 1024, 1024), generator=generator, dtype=torch.float32)
...
def _run_case(case: BenchCase) -> BenchResult:
    ...
    image = _make_image(device)
    landmarks = canonical_landmarks(target_size=1024, padding=0.2).to(device=device)
    measure_once = _latency_runner(device)

    def run_once(image_tensor: torch.Tensor = image) -> tuple[torch.Tensor, torch.Tensor]:
        return facial_align(
            image_tensor,
            landmarks,
            target_size=case.target_size,
...
def render_markdown(hardware: dict[str, str], results: list[BenchResult]) -> str:
    lines = [
        ...
        "## Cases",
        "",
        "| case | device | target_size | status | reason | min_ms | median_ms | mean_ms |",
        "|---|---|---:|---|---|---:|---:|---:|",
    ]

    for row in results:
        lines.append(
            f"| {row.name} | {row.device} | {row.target_size} | {row.status} | "
...
def test_bench_facial_aligner(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
...
def main() -> int:
    ...
    logging.getLogger("core.facial_aligner").setLevel(logging.ERROR)
