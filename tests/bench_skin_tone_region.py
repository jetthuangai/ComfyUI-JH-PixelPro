...
from core.skin_tone_region import skin_tone_tri_region  # noqa: E402
...
SEED = 20260424
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_skin_tone_region.json"
...
CASES = [
    BenchCase("cpu-512-full", 512, False),
    BenchCase("cpu-512-mask", 512, True),
    BenchCase("cpu-1024-full", 1024, False),
]
...
def _make_image(resolution: int) -> torch.Tensor:
    ...
    return torch.rand((1, 3, resolution, resolution), generator=generator, dtype=torch.float32)


def _make_mask(resolution: int) -> torch.Tensor:
    yy, xx = torch.meshgrid(
        torch.linspace(-1.0, 1.0, resolution),
        torch.linspace(-1.0, 1.0, resolution),
        indexing="ij",
    )
    return ((xx * xx + yy * yy) <= 0.65).to(torch.float32).unsqueeze(0)
...
def _run_case(case: BenchCase) -> BenchResult:
    ...
    skin_mask = _make_mask(case.resolution) if case.with_mask else None

    def run_once(
        image_bchw: torch.Tensor = image,
        mask_bhw: torch.Tensor | None = skin_mask,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return skin_tone_tri_region(image_bchw, skin_mask=mask_bhw)
...
def test_bench_skin_tone_region(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
