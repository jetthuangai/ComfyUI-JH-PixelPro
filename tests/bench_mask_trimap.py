...
from core.mask_trimap import build_trimap  # noqa: E402

SEED = 20260421
WARMUP_ITERS = 3
MEASURE_ITERS = 10
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_mask_trimap.json"
...
class BenchCase:
    ...
    @property
    def name(self) -> str:
        return f"cpu-{self.resolution}-b{self.batch}-trimap"
...
def _run_case(case: BenchCase) -> dict[str, float | int | str]:
    ...
    def run_once() -> torch.Tensor:
        return build_trimap(mask, fg_radius=4, bg_radius=8, smoothing=0.0)
...
def test_bench_mask_trimap(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
