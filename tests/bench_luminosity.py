...
from core import luminosity_masks  # noqa: E402
...
BASELINE_PATH = Path(__file__).with_name("bench_baselines") / "bench_luminosity.json"
...
CASES = [
    BenchCase("cpu-512-lab", "cpu", 512, "lab_l", 2, 10),
    BenchCase("cpu-1024-lab", "cpu", 1024, "lab_l", 1, 10),
    BenchCase("cpu-2048-lab", "cpu", 2048, "lab_l", 0, 10),
    BenchCase("cpu-1024-ycbcr", "cpu", 1024, "ycbcr_y", 1, 10),
    BenchCase("cpu-1024-hsv", "cpu", 1024, "hsv_v", 1, 10),
    BenchCase("gpu-512-lab", "cuda", 512, "lab_l", 2, 10),
    BenchCase("gpu-1024-lab", "cuda", 1024, "lab_l", 1, 10),
    BenchCase("gpu-2048-lab", "cuda", 2048, "lab_l", 0, 10),
]
...
def _run_case(case: BenchCase) -> BenchResult:
    ...
    def run_once(
        ...
        return luminosity_masks(image_tensor, luminance_source=case.source, soft_edge=0.1)
...
def render_markdown(hardware: dict[str, str], results: list[BenchResult]) -> str:
    lines = [
        ...
        "## Cases",
        "",
        "| case | device | resolution | source | status | reason | min_ms | median_ms | mean_ms |",
        "|---|---|---:|---|---|---|---:|---:|---:|",
    ]

    for row in results:
        lines.append(
            f"| {row.name} | {row.device} | {row.resolution} | {row.source} | "
...
def test_bench_luminosity(case: BenchCase, capsys: pytest.CaptureFixture[str]) -> None:
...
def main() -> int:
    ...
    logging.getLogger("core.luminosity").setLevel(logging.ERROR)
